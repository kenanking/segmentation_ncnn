  ...
  if (topk > 1) {
        ...
        for (int col = 0; col < bottom_blobs[0].w; col++) {
          ...
          if (largest == 1) {
            for (int i = 0; i < bottom_blobs[0].c; i++) {
              vec[i] =
                  std::make_pair(bottom_blobs[0].channel(i).row(r)[col], -i);
            }
            std::partial_sort(vec.begin(), vec.begin() + topk, vec.end(),
                              std::greater<std::pair<float, int>>());
          } else if (largest == 0) {
            ...
            if (largest == 1) {
              for (int i = 0; i < bottom_blobs[0].c; i++) {
                ...
                if (bottom_blobs[0].channel(i).row(r)[col] > valtarget) {
                  ...
                  cur++;
                } else if (bottom_blobs[0].channel(i).row(r)[col] ==
                               valtarget &&
                           i <= indtarget) {
                  top_val_blob.channel(cur).row(r)[col] =
                      bottom_blobs[0].channel(i).row(r)[col];
            ...
            if (largest == 1) {
              for (int i = 0; i < bottom_blobs[0].w; i++) {
                ...
                if (bottom_blobs[0].channel(page).row(r)[i] > valtarget) {
                  ...
                  cur++;
                } else if (bottom_blobs[0].channel(page).row(r)[i] ==
                               valtarget &&
                           i <= indtarget) {
                  top_val_blob.channel(page).row(r)[cur] =
                      bottom_blobs[0].channel(page).row(r)[i];
    ...
    if (dims == 1 && positive_axis == 0) {
      ...
      if (largest == 1) {
        ...
        if (top_blobs.size() == 2)
          ...
          valptr[0] =
              std::distance(vec.begin(), index_iter); // replace with index
      } else if (largest == 0) {
    ...
    if (dims == 3 && positive_axis == 0) {
      if (keep_dims == 1) {
        ...
        top_val_blob.create(bottom_blobs[0].w, bottom_blobs[0].h, 4u,
                            opt.blob_allocator);
        if (top_val_blob.empty())
          return -100;
        if (top_blobs.size() == 2) {
          top_ind_blob.create(bottom_blobs[0].w, bottom_blobs[0].h, 4u,
                              opt.blob_allocator);
          if (top_ind_blob.empty())
            return -100;
        }
      ...
      for (int r = 0; r < bottom_blobs[0].h; r++) {
        for (int col = 0; col < bottom_blobs[0].w; col++) {
          ...
          if (largest == 1) {
            ...
            if (top_blobs.size() == 2)
              indptr[r * top_ind_blob.w + col] =
                  std::distance(vec.begin(), index_iter);
            else
              valptr[r * top_ind_blob.w + col] =
                  std::distance(vec.begin(), index_iter);
    ...
    if (dims == 3 && positive_axis == 1) {
      if (keep_dims == 1) {
        ...
        for (int page = 0; page < bottom_blobs[0].c; page++) {
          ...
          float *indptr;
          if (top_blobs.size() == 2)
            indptr = top_ind_blob.channel(page);
          for (int col = 0; col < bottom_blobs[0].w; col++) {
            for (int i = 0; i < bottom_blobs[0].h; i++) {
              vec[i] = ptr[i * bottom_blobs[0].w + col];
            }
            if (largest == 1) {
              ...
              fprintf(stderr,
                      "largest attribute should be 0 or 1, but not %d\n",
                      largest);
              return -100;
            }
          }
        ...
        top_val_blob.create(bottom_blobs[0].w, bottom_blobs[0].c, 4u,
                            opt.blob_allocator);
        if (top_val_blob.empty())
          return -100;
        if (top_blobs.size() == 2) {
          top_ind_blob.create(bottom_blobs[0].w, bottom_blobs[0].c, 4u,
                              opt.blob_allocator);
          if (top_ind_blob.empty())
            return -100;
        }
        ...
        for (int page = 0; page < bottom_blobs[0].c; page++) {
          ...
          for (int col = 0; col < bottom_blobs[0].w; col++) {
            ...
            if (largest == 1) {
              ...
              if (top_blobs.size() == 2)
                indptr[page * top_ind_blob.w + col] =
                    std::distance(vec.begin(), index_iter);
              else
                valptr[page * top_ind_blob.w + col] =
                    std::distance(vec.begin(), index_iter);
            } else if (largest == 0) {
              auto index_iter = std::min_element(vec.begin(), vec.end());
              valptr[page * top_val_blob.w + col] = *index_iter;
    ...
    if (dims == 3 && positive_axis == 2) {
      if (keep_dims == 1) {
        ...
        top_val_blob.create(bottom_blobs[0].h, bottom_blobs[0].c, 4u,
                            opt.blob_allocator);
        if (top_val_blob.empty())
          return -100;
        if (top_blobs.size() == 2) {
          top_ind_blob.create(bottom_blobs[0].h, bottom_blobs[0].c, 4u,
                              opt.blob_allocator);
          if (top_ind_blob.empty())
            return -100;
        }
        ...
        for (int page = 0; page < bottom_blobs[0].c; page++) {
          ...
          for (int r = 0; r < bottom_blobs[0].h; r++) {
            ...
            if (largest == 1) {
              ...
              if (top_blobs.size() == 2)
                indptr[page * top_ind_blob.w + r] =
                    std::distance(vec.begin(), index_iter);
              else
                valptr[page * top_ind_blob.w + r] =
                    std::distance(vec.begin(), index_iter);
            } else if (largest == 0) {
              auto index_iter = std::min_element(vec.begin(), vec.end());
              valptr[page * top_val_blob.w + r] = *index_iter;
              if (top_blobs.size() == 2)
                indptr[page * top_val_blob.w + r] =
                    std::distance(vec.begin(), index_iter);
              else
                valptr[page * top_ind_blob.w + r] =
                    std::distance(vec.begin(), index_iter);
